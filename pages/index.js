...
export default class App extends React.Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      if (network.chainId === 4 && network.name === "rinkeby") {
        this.setState({
          provider,
          message: "Pass the captcha to mint one NFT"
        });
      } else {
  ...
  executeMint = async (hash, signature) => {
    ...
    this.setState({
      message: "Good to go! Accept the minting tx on Metamask."
    });
    try {
      ...
      alert(
        `Success! You minted a new NFT: https://rinkeby.etherscan.io/tx/${receipt.transactionHash}`
      );
    } catch (error) {
    ...
    return (
      <div className={styles.container}>
        <main className={styles.main}>
          <h1 className={styles.title}>NFT minter app for humans</h1>
          ...
          <p>
            <a
              href="https://github.com/tinchoabbate/nft-minter-for-humans"
              target="_blank"
            >
              What's this site?
            </a>
